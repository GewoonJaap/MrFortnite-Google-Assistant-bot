...
app.post('/webhook', (req, res) => {
  ...
  if (intent == 'FunFacts') {
    ...
    res.status(200).json({
      fulfillmentText: strings[locale].facts[fact] + strings[locale].AnythingElse,
      source: 'Mr. Fortnite backend'
    });
  ...
  function commandlogging() {
    ...
    request.post(CommandLoggerURL, {
        form: {
          'entry.1325239568': intent,
          'entry.761812201': Text
        }
      },
  ...
  if (intent == 'Feedback') {
    ...
    request.post(GoogleDriveFeedbackURL, {
        form: {
          'entry.180196638': intenttext,
          'entry.1913027449': whatwould,
          'entry.2083165757': error
        }
      },
      function (error, response, body) {
